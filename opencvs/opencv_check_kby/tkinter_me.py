...
def trackbar(win_name,test_img,add_img):
    ...
    # 윈도우 창 생성
    cv2.imshow(win_name, test_img) # 트랙바 크기를 새로운 img크기에 맞추기 위해 필요한 코드
...
def run():
    if (e1.get()=='')|(e2.get()==''):
        ...
        try:
            test_file_path = "imgtrain/jk/"+e1.get()+".jpg"
            add_file_path = "imgtrain/jk/"+e2.get()+".jpg"

            test_array=np.fromfile(test_file_path,np.uint8)
            test_img=cv2.imdecode(test_array,-1)

            add_array=np.fromfile(add_file_path,np.uint8)
            ...
            # test img에서 얼굴 인식하기
            test_copy_img = np.copy(test_img) # 이미지 복사
...
fc_window=Tk()
fc_window.geometry('390x70+100+500')
...
# 텍스트 입력 창
e1=Entry(fc_window, width=30)
e2=Entry(fc_window, width=30)
e1.grid(row=0, column=1)
e2.grid(row=1, column=1)

# 버튼 만들기
b1 = Button(fc_window, text='실행', command=run,  width=13)
b2= Button(fc_window, text='초기화', command=reset,  width=13)
