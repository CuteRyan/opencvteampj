...
def tie_img(test_img,add_img):
    ...
    roi = test_img[10:add_rows+10,test_cols-5-add_cols:test_cols-5] #로고파일 필셀값을 관심영역(ROI)으로 저장함
    ...
    test_img[10:add_rows+10,test_cols-5-add_cols:test_cols-5] = result_img

    result = test_img
    return result


# 4-3. 합성된 사진 명암 조절 및 저장하는 함수
def onChange(pos):
    pass
def brightness(win_name,img):
    # 윈도우 창 생성
    # cv2.imshow(win_name, img)
    cv2.namedWindow(win_name)

    # 트랙바 생성(포스에서 소수점자리 안되므로 일단은 정수로 표현후, 추후에 sigma_r 100으로 나눔)
    cv2.createTrackbar('brightness', win_name, 0, 40, onChange)

    # 트랙바 디폴트값 설정
    cv2.setTrackbarPos('brightness', win_name, 20)

    while True:
        try:
            k = cv2.getTrackbarPos('brightness', win_name)
            if k>20: # 밝게 만드는 함수
                k=k-20
                t = (k*5, k*5, k*5)
...
def run(models, test_img, add_img):
    ...
    try:
        ...
        if acc_score > 50:
            ...
            # 사진에 add_img 합성
            result_img = tie_img(test_copy_img, add_img)
            # 밝기조절 함수, s키를 누르면 저장, q키를 누르면 종료
            save_img = brightness('!!!!!double click :: save=s, quit=q!!!!!', result_img)
...
# %%
test_img = cv2.imread("./imgtrain/jk/3.jpg")
